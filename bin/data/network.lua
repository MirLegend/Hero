...
function onZiyuRecv()
	print("onZiyuRecv -----------------------------X-----------------------XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
end

function sendEx(buff)
     local str ="POST / HTTP/1.1\r\nHost: 120.78.66.35\r\n";
 
        str=str.."Content-type: application/x-www-as-";
 
        str=str.."urlencoded\r\n";
 
        str=str.."User-Agent: Mozilla 4.0\r\nContent-length: "..string.len(buff);
...
function doLoginProxy()
	...
	if isLoginFirst then
		...
		--print("isLoginFirst "..buf)
		print("isLoginFirst userId.."..userId)
		print("isLoginFirst serverId.."..serverId)
		print(print_bytes(buf))
		local r1, r2 = ziyu.send(myip, myport, buf)--connection:send(buf)
		if not r1 then
			print("SENDING_FAILED_PLEASE_CHECK_THE_NETWORK_SETTING")
...
local function connect(block)
	...
	if not connect_succ then
		...
		print("connection:connect ip "..game_server_ip)
		print("connection:connect port "..game_server_port)
	...
	if connect_succ then
		print("[netword.lua|connect] connect succ")
		...
		if r1 then
			print("[network.lua|connect]  Message Sent  @ " .. ed.getYMDTime())
		else
			print(string.format("[network.lua|connect]  ERROR | connection:send failed | %s", r2))
			close()
		end
		--close()
	elseif block then
		print(string.format("[network.lua|connect]  ERROR | Blocked Connect failed | %s", err))
		return false
	else
		print("[netword.lua|connect] connect start")
		return true
	end
end
...
local function recv()
	...
	print("recv------------------------------------errorCode: "..errorCode);
	if errorCode ~= 0 then
		close()
		if errorCode == 3 then
			reloadProcess()
	...
	if buffer then
		print("[netword.lua|recv]  Data Received @ " .. ed.getYMDTime())
		loadEnd()
