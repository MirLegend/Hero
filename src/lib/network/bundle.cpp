...
void Bundle::newMessage(const MessageHandler& msgHandler)
{
	...
	uint8 mainCmd = msgHandler.msgID >> 8;
	uint8 subCmd = (uint8)msgHandler.msgID;
	(*this) << mainCmd;
	(*this) << subCmd;
...
void Bundle::newMessage(uint8 mainCmd, uint8 subCmd)
{
	...
	(*this) << mainCmd;
	(*this) << subCmd;
	pCurrPacket_->messageID(MSGIDMAKE(mainCmd, subCmd)/*((uint16)mainCmd<<16) | subCmd*/);
	WARNING_MSG(fmt::format("Bundle::newMessage: cmd({},{}) bundle length:{}!\n",
		mainCmd, subCmd, currMsgLength_));

	++numMessages_;
	currMsgID_ = MSGIDMAKE(mainCmd, subCmd)/*msgHandler.msgID*/;
	currMsgPacketCount_ = 0;
}
