...
bool Resmgr::initialize()
{
	...
	if (!xml->isGood())
	{
		ERROR_MSG(fmt::format("ServerConfig::serverConfig: load {} is failed!\n",
			loadconfig.c_str()));
		return false;
	}
	node = xml->getRootNode("serverpaths");
	if (node != NULL)
	{
		TiXmlNode* childnode = xml->enterNode(node, "root_path");
		if (childnode)
		{
			kb_env_.root = xml->getValStr(childnode);
		}

		childnode = xml->enterNode(node, "res_path");
		if (childnode)
		{
			kb_env_.res_path = xml->getValStr(childnode);
		}

		childnode = xml->enterNode(node, "bin_path");
		if (childnode)
		{
			kb_env_.bin_path = xml->getValStr(childnode);
		}
	}
...
{
	INFO_MSG(fmt::format("Resmgr::initialize: MINI_KBE_ROOT={0}\n", kb_env_.root));
	INFO_MSG(fmt::format("Resmgr::initialize: MINI_KBE_RES_PATH={0}\n", kb_env_.res_path));
	INFO_MSG(fmt::format("Resmgr::initialize: MINI_KBE_BIN_PATH={0}\n", kb_env_.bin_path));

#if KBE_PLATFORM == PLATFORM_WIN32
	printf("%s", fmt::format("MINI_KBE_ROOT = {0}\n", kb_env_.root).c_str());
	printf("%s", fmt::format("MINI_KBE_RES_PATH = {0}\n", kb_env_.res_path).c_str());
	printf("%s", fmt::format("MINI_KBE_BIN_PATH = {0}\n", kb_env_.bin_path).c_str());
	printf("\n");
#endif
}
